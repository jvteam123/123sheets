document.addEventListener('DOMContentLoaded', () => {
    ...
    ProjectTrackerApp.methods = {
        ...
        attachEventListeners() {
            ...
            attachClick(self.elements.clearDataBtn, () => self.methods.handleClearData.call(self));
            attachClick(self.elements.nextPageBtn, () => self.methods.handleNextPage.call(self));
            attachClick(self.elements.prevPageBtn, () => self.methods.handlePrevPage.call(self));
            attachClick(self.elements.nextDisputePageBtn, () => self.methods.handleNextDisputePage.call(self));
            attachClick(self.elements.prevDisputePageBtn, () => self.methods.handlePrevDisputePage.call(self));
            ...
            if (self.elements.userManagementForm) {
                self.elements.userManagementForm.addEventListener('submit', (event) => self.methods.handleUserFormSubmit.call(self, event));
            }
            if (self.elements.disputeForm) {
                self.elements.disputeForm.addEventListener('submit', (event) => self.methods.handleDisputeFormSubmit.call(self, event));
            }
            ...
            if (self.elements.disputeTableBody) {
                self.elements.disputeTableBody.addEventListener('click', (event) => self.methods.handleDisputeTableActions.call(self, event));
            }
            attachClick(self.elements.importUsersBtn, () => self.elements.userCsvInput.click());
            attachClick(self.elements.exportUsersBtn, () => self.methods.handleExportUsers.call(self));
            if (self.elements.userCsvInput) {
                self.elements.userCsvInput.onchange = (event) => self.methods.handleImportUsers.call(self, event);
            }
    ...
    if (!this.state.isAppInitialized) {
        ...
        async renderTLDashboard() {
            ...
            settingsDiv.querySelector('#saveMeetingLinkBtn').onclick = () => this.methods.handleSaveMeetingLink.call(this);
            settingsDiv.querySelector('#saveTscLinkBtn').onclick = () => this.methods.handleSaveTscLink.call(this);
...
Status: ${dispute.status}
    ...
    ProjectTrackerApp.init = () => {
        try {
            ...
            ProjectTrackerApp.methods.injectDisputeModalHTML.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.setupDOMReferences.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.injectNotificationStyles.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.injectTechIdHintStyles.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.loadColumnVisibilityState.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.setupAuthRelatedDOMReferences.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.attachEventListeners.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.setupTechIdHint.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.setupAuthActions.call(ProjectTrackerApp);
            ProjectTrackerApp.methods.listenForAuthStateChanges.call(ProjectTrackerApp);
        } catch (error) {
